...
import { getExportData } from 'apiSdk/export-data';
import { ExportDataInterface } from 'interfaces/export-data';
import { getGraphics } from 'apiSdk/graphics';
import { GraphicInterface } from 'interfaces/graphic';
import { getDataFilters } from 'apiSdk/data-filters';
import { DataFilterInterface } from 'interfaces/data-filter';

import { DataFilterListPage as ShowCaseList } from 'pages/data-filters';

function DashboardPage() {
  const { session } = useSession();

  const exportDataStat = useSWR<PaginatedInterface<ExportDataInterface>>(
    () => `/export-data`,
    () => getExportData({ limit: 1, offset: 0 }),
  );

  const graphicStat = useSWR<PaginatedInterface<GraphicInterface>>(
    () => `/graphics`,
    () => getGraphics({ limit: 1, offset: 0 }),
  );

  const dataFilterStat = useSWR<PaginatedInterface<DataFilterInterface>>(
    () => `/data-filters`,
    () => getDataFilters({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Export Data',
      result: exportDataStat?.data?.totalCount,
      isLoading: exportDataStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Graphics',
      result: graphicStat?.data?.totalCount,
      isLoading: graphicStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Data Filters',
      result: dataFilterStat?.data?.totalCount,
      isLoading: dataFilterStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`data-filters`} passHref legacyBehavior>
                  <Button
