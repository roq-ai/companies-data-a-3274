...
import { getDataFilterById, updateDataFilterById } from 'apiSdk/data-filters';
...
function DataFilterEditPage() {
  ...
  const handleSubmit = async (values: DataFilterInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateDataFilterById(id, values);
      mutate(updated);
      resetForm();
